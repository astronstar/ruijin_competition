...
def combine_relations(sent, left_list, right_list, length):  # 将关系配对
    ...
    for left in left_list:  # mei ju jie shu append rel (entity_id, entity_type, start, end)
        ...
        for right in right_list:
            ...
            if startl < startr:
                if len(sent) > 80:
                    if len(sent[endl:startr]) > 80:  # 中间很长
                        mid = re.sub('。.*。', '', sent[endl:startr])
                        s = f'{sent[max(startl-20,0):startl]}<e1>{sent[startl:endl]}</e1>{mid}<e2>{sent[startr:endr]}</e2>{sent[endr:min(endr+20,len(sent))]}'
                    else:
                        s = f'{sent[max(startl-20,0):startl]}<e1>{sent[startl:endl]}</e1>{sent[endl:startr]}<e2>{sent[startr:endr]}</e2>{sent[endr:min(endr+20,len(sent))]}'
                else:
                    s = f'{sent[0:startl]}<e1>{sent[startl:endl]}</e1>{sent[endl:startr]}<e2>{sent[startr:endr]}</e2>{sent[endr:]}'

            else:
                if len(sent) > 80:
                    if len(sent[endr:startl]) > 80:
                        mid = re.sub('。.*。', '', sent[endr:startl])
                        s = f'{sent[max(startr-20,0):startr]}<e1>{sent[startr:endr]}</e1>{mid}<e2>{sent[startl:endl]}</e2>{sent[endl:min(endl+20,len(sent))]}'
                    else:
                        s = f'{sent[max(startr-20,0):startr]}<e1>{sent[startr:endr]}</e1>{sent[endr:startl]}<e2>{sent[startl:endl]}</e2>{sent[endl:min(endl+20,len(sent))]}'
                else:
