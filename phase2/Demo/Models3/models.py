...
class AttRNN():
    ...
    def __init__(
        ...
        with tf.name_scope('add'):
            ...
            # self.inputs = inception(self.inputs, num_filters=self.hidden_size,scope='in')

            # self.inputs = res_block(self.inputs, self.hidden_size, 1)
            # self.inputs = tf.layers.dense(self.inputs, self.hidden_size, activation=tf.nn.relu, use_bias=True,
            ...
            h2 = tf.layers.conv1d(
                inputs=outputs, filters=num_filters, kernel_size=filter_size2, activation=None, use_bias=True,
                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                bias_initializer=tf.constant_initializer(0.01)
            )
            h3 = tf.layers.conv1d(
                inputs=outputs, filters=num_filters, kernel_size=filter_size3, activation=None, use_bias=True,
                kernel_initializer=tf.contrib.layers.xavier_initializer(),
                bias_initializer=tf.constant_initializer(0.01)
            )
            ...
            pooled2 = tf.layers.max_pooling1d(h2, sequence_length - filter_size2 + 1, 1)
            pooled3 = tf.layers.max_pooling1d(h3, sequence_length - filter_size3 + 1, 1)

            pooled_flatten = tf.tanh(tf.reshape(pooled, [-1, num_filters]))
            pooled_flatten1 = tf.tanh(tf.reshape(pooled1, [-1, num_filters]))
            pooled_flatten2 = tf.tanh(tf.reshape(pooled2, [-1, num_filters]))
            pooled_flatten3 = tf.tanh(tf.reshape(pooled3, [-1, num_filters]))

            self.pooled_flatten = tf.concat([pooled_flatten, pooled_flatten1,pooled_flatten2,pooled_flatten3], axis=-1)

            # classification
            self.Embedding_C = tf.get_variable(
                'embedding_class',
                shape=[4 * num_filters, num_classes],
                initializer=tf.contrib.layers.xavier_initializer(uniform=True)
            )
