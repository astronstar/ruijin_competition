...
class DataLoader():
    ...
    def __init__(self, max_sequence_length=500):
        if os.path.exists(r'../DataSets/mydatae/train_instances.pkl') and os.path.exists(
                r'../DataSets/mydatae/test_txtid.pkl') and os.path.exists(
                r'../DataSets/mydatae/test_instances.pkl') and os.path.exists(
                r'../DataSets/mydatae/word2id_dic.pkl'):
            print('loading pkl...')
            train_instances_pkl = open(r'../DataSets/mydatae/train_instances.pkl', 'rb')
            train_instances = pickle.load(train_instances_pkl)
            test_txtid_pkl = open(r'../DataSets/mydatae/test_txtid.pkl', 'rb')
            test_txtid = pickle.load(test_txtid_pkl)
            test_instances_pkl = open(r'../DataSets/mydatae/test_instances.pkl', 'rb')
            test_instances = pickle.load(test_instances_pkl)

            word2id_dic_pkl = open(r'../DataSets/mydatae/word2id_dic.pkl', 'rb')
        ...
        # # train set
        if not os.path.exists(r'../DataSets/mydatae/train_set.pkl'):
            self.train_dataset = self.get_train_set(train_instances)
            train_set_pkl = open(r'../DataSets/mydatae/train_set.pkl', 'wb')
            pickle.dump(self.train_dataset, train_set_pkl)
        else:
            train_set_pkl = open(r'../DataSets/mydatae/train_set.pkl', 'rb')
            self.train_dataset = pickle.load(train_set_pkl)
        # # test set
        if not os.path.exists(r'../DataSets/mydatae/test_set.pkl'):
            self.test_dataset = self.get_test_set(test_txtid, test_instances)
            test_set_pkl = open(r'../DataSets/mydatae/test_set.pkl', 'wb')
            pickle.dump(self.test_dataset, test_set_pkl)
        else:
            test_set_pkl = open(r'../DataSets/mydatae/test_set.pkl', 'rb')
        ...
        #     self.train_dataset)
        self.train_set_min1 = self.get_train_each(self.train_dataset,relation_type='Anatomy')
        self.test_set_min1 = self.get_test_each(self.test_dataset,relation_type='Anatomy')
    ...
    def get_train_each(self, train_set,relation_type):
        ...
        train_set_min=train_set_min+pos_list*1
    ...
    def get_testb_instance(self):
        if os.path.exists(r'../DataSets/mydatae/testb_txtid.pkl') and os.path.exists(
                r'../DataSets/mydatae/testb_instances.pkl'):
            testb_txtid_pkl = open(r'../DataSets/mydatae/testb_txtid.pkl', 'rb')
            testb_txtid = pickle.load(testb_txtid_pkl)
            testb_instances_pkl = open(r'../DataSets/mydatae/testb_instances.pkl', 'rb')
    ...
    def get_words_pos_y(self, instance):
        ...
        words_before = ''.join([word for word in words_before if word not in self.stop_words])
        words_middle = ''.join([word for word in words_middle if word not in self.stop_words])
        words_after = ''.join([word for word in words_after if word not in self.stop_words])
...
if __name__ == '__main__':
    ...
    t=dataloader.test_set_min1
    print(len(t))
    tt = dataloader.train_set_min1
    for (txtid,instances) in t:
        print(txtid)
        # print(instances)
        try:
        ...
        #     print(instance)
        # for (tag, ids, p1, p2, y) in instances:
        #     print(tag,y)



    # for i, instance in enumerate(dataloader.train_set_min1):
    #     if i % 500 == 0:
    #         print(i)
    #         print(instance)
    # print(i)
