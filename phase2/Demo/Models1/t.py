...
num = 'cnneTest'
LOG_FILE = num + '_result.log'
handler = logging.handlers.RotatingFileHandler(LOG_FILE, maxBytes=1024 * 1024, backupCount=5)  # 实例化handler
fmt = '%(asctime)s : %(message)s'

formatter = logging.Formatter(fmt)  # 实例化formatter
handler.setFormatter(formatter)  # 为handler添加formatter
logger = logging.getLogger('result')  # 获取名为result的logger
if logger.handlers:
...
with tf.Graph().as_default():
    ...
    with tf.Session(config=session_conf) as sess:
        ...
        dev_data_list = dev_input_queue(dev_data, batch_size=dev_batch_size)
            ...
            for epoch in range(1, num_epochs + 1):
                for step in range(num_batches):
                    ...
                    if step % 100 == 0:
                        ...
                        #                                                                           acc))
                        logger.info(
                            'epoch {}, step {}, cost {}, acc {}'.format(epoch, (epoch - 1) * num_batches + step, cost,
                                                                        acc))

                learning_rate = (learning_rate / float(epoch)) if learning_rate > 0.00001 else 0.00001
                print('\ndev...')
                pre = 0
                times = math.ceil(len(dev_data) / dev_batch_size)
